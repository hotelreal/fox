...
class RiderViewController: UIViewController, CLLocationManagerDelegate {
		...
		if !isCallingUber {
			...
			riderRequest.saveInBackgroundWithBlock { (succsess, error) -> Void in
				if error == nil {
					...
					self.uberButton.setTitle("Cancelar MiRaite", forState: UIControlState.Normal)
				}
				else {
					self.displayAlert("No se puede localizar MiRaite", message: "Intentar mas tarde")
				}
			}
		}
		else {
			...
			query.findObjectsInBackgroundWithBlock({ (objects, error) -> Void in
				if error != nil {
					print("Error no se localiza MiRaite")
				}
				else {
			...
			self.uberButton.setTitle("Llama a MiRaite", forState: UIControlState.Normal)
			isCallingUber = false
		}
	}
	...
	func locationManager(manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
		...
		if isCallingUber {
			...
			query.findObjectsInBackgroundWithBlock({ (objects, error) -> Void in
				if error != nil {
					print("Error localizando pasajero")
				}
				else {
					if let objects = objects {
						for object in objects {
											...
											if let drivers = drivers {
												for driver in drivers {
													if let driverLocation = driver["driverLocation"] as? PFGeoPoint {
														...
														self.uberButton.setTitle("El conductor esta a \(distance)km de distancia!", forState: UIControlState.Normal)
														self.isDriverOnTheWay = true
													}
												}
											}
										}
										else { print("No puedo localizar conductor") }
									})
								}
							}
						}
